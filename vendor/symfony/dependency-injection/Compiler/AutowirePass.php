...
use SmokeTestgen202507\Symfony\Component\DependencyInjection\Attribute\Autowire;
use SmokeTestgen202507\Symfony\Component\DependencyInjection\Attribute\AutowireCallable;
use SmokeTestgen202507\Symfony\Component\DependencyInjection\Attribute\AutowireDecorated;
use SmokeTestgen202507\Symfony\Component\DependencyInjection\Attribute\MapDecorated;
...
use SmokeTestgen202507\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException;
...
use SmokeTestgen202507\Symfony\Component\VarExporter\ProxyHelper;
/**
...
class AutowirePass extends AbstractRecursivePass
{
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            if ($checkAttributes) {
                foreach (\method_exists($parameter, 'getAttributes') ? $parameter->getAttributes(Autowire::class, \ReflectionAttribute::IS_INSTANCEOF) : [] as $attribute) {
                    ...
                    try {
                        $value = $this->processValue(new TypedReference($type ?: '?', $type ?: 'mixed', $invalidBehavior, $name, \array_merge([$attribute], $target)));
    ...
    private function getAutowiredReference(TypedReference $reference, bool $filterType) : ?TypedReference
    {
        ...
        $name = $target = ($nullsafeVariable1 = \array_filter($reference->getAttributes(), static function ($a) {
        ...
        if (null !== ($name = $name ?? $reference->getName())) {
            ...
            if ($this->container->has($n = $name) && !$this->container->findDefinition($n)->isAbstract() || $this->container->has($n = $parsedName) && !$this->container->findDefinition($n)->isAbstract()) {
                foreach ($this->container->getAliases() as $id => $alias) {
                    if ($n === (string) $alias && \strncmp($id, $type . ' $', \strlen($type . ' $')) === 0) {
    ...
    private function createTypeNotFoundMessage(TypedReference $reference, string $label, string $currentId) : string
    {
        ...
        if (!($r = $this->container->getReflectionClass($type, \false))) {
            ...
            if (null !== ($target = \array_filter($reference->getAttributes(), static function ($a) {
                return $a instanceof Target;
            })[0] ?? null)) {
    ...
    private function populateAutowiringAlias(string $id, ?string $target = null) : void
    {
        ...
        if (\class_exists($type, \false) || \interface_exists($type, \false)) {
            if (null !== $target && \strncmp($target, '.' . $type . ' $', \strlen('.' . $type . ' $')) === 0 && (new Target($target = \substr($target, \strlen($type) + 3)))->getParsedName() === $name) {
                $name = $target;
            }
